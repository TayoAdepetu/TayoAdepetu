    ...
    <section id="about" className="relative py-24 px-4 sm:px-6 lg:px-8 overflow-hidden">
      <div className="max-w-5xl mx-auto">
        ...
        >
          ...
          <h2 className="mt-4 text-2xl sm:text-3xl lg:text-4xl font-bold text-slate-900 dark:text-slate-50 leading-tight max-w-3xl">
            Software Engineer by day. Writer by instinct. <span className="text-gradient-brand">SEO nerd for life.</span>
          </h2>
        </motion.div>

        <div className="grid lg:grid-cols-[1.5fr_1fr] gap-10 mt-10 items-start">
          <motion.div
            initial={{ opacity: 0, y: 20 }}
            whileInView={{ opacity: 1, y: 0 }}
            viewport={{ once: true, margin: '-80px' }}
            transition={{ duration: 0.6, delay: 0.1 }}
            className="space-y-4 text-base text-slate-600 dark:text-slate-300 leading-relaxed"
          >
            <p>
              I&apos;m a <strong className="text-slate-900 dark:text-slate-100">Full-stack Software Engineer</strong>{' '}
              with 4+ years of shipping production software. I&apos;ve built enterprise systems, fintech payment rails, AI-powered claims processors, and
            ...
            <p>
              ...
              <strong className="text-slate-900 dark:text-slate-100">Nigeria and globally (US, UK, Germany, Australia)</strong> —
          ...
          >
            {FACTS.map(({ Icon, label, value }) => (
              <div
                key={label}
                className="flex items-center gap-3 p-3.5 rounded-xl bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-800"
              >
                <div className="flex-shrink-0 h-10 w-10 rounded-lg bg-brand-100 dark:bg-brand-900/40 text-brand-700 dark:text-brand-300 flex items-center justify-center">
            ...
            <div className="p-4 rounded-xl bg-gradient-to-br from-brand-600 to-brand-800 text-white">
              <GraduationCap className="h-4 w-4 mb-1.5 opacity-80" />
              <div className="text-[10px] uppercase tracking-wider opacity-80">Now learning</div>
              <div className="text-sm font-semibold mt-0.5">
                B.Sc Computer Science · Miva Open University
              </div>
              <div className="text-[11px] opacity-80 mt-1">Plus Web3Bridge smart contract cohort X.</div>
            </div>

            <div className="p-4 rounded-xl bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-800">
              <Search className="h-4 w-4 text-brand-600 dark:text-brand-400 mb-1.5" />
              <div className="text-[10px] uppercase tracking-wider text-slate-500 dark:text-slate-400">
                Dual specialism
              </div>
